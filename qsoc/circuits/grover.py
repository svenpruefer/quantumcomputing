...
def add_grover_reflection_no_ancilla(qc: QuantumCircuit, register: QuantumRegister) -> None:
    ...
    qc.x(register)
    qc.h(register[0])
    qc.mct(control_qubits=register[1:], target_qubit=register[0], ancilla_qubits=None, mode='noancilla')
    qc.h(register[0])
    qc.x(register)
    qc.h(register)
...
def add_grover_reflection_with_ancilla(qc: QuantumCircuit, register: QuantumRegister, ancillas: List[Qubit]) -> None:
    ...
    qc.x(register)
    qc.h(register[0])
    qc.mct(control_qubits=register[1:], target_qubit=register[0], ancilla_qubits=ancillas)
    qc.h(register[0])
    qc.x(register)
    qc.h(register)
...
def add_grover_reflection_with_ancilla_on_registers(qc: QuantumCircuit, registers: Set[QuantumRegister],
    ...
    for qubit in qubits:
        ...
        qc.x(qubit)
    qc.h(qubits[0])
    qc.mct(control_qubits=qubits[1:], target_qubit=qubits[0], ancilla_qubits=list(ancillas))
    qc.h(qubits[0])
    for qubit in qubits:
        qc.x(qubit)
        qc.h(qubit)
